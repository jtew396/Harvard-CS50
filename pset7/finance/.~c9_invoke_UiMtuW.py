...
def index():
    ...
    if request.method == "GET":
        ...
        stock_symbols = db.execute("SELECT stock FROM portfolio WHERE id = 1 GROUP BY stock")
        ...
        for stock in stock_symbols:
            ...
            stock_shares = db.execute("SELECT shares FROM portfolio WHERE id = 1 AND stock = :stock_symbol", stock_symbol = stock_symbol)
        ...
        user_cash = float(db.execute("SELECT cash FROM users WHERE id = 1"))

        # Calculate the total value of user's cash and stocks
        total_value = float(user_cash) + float(stock_value_sum)

        return render_template("index.html", total_value=total_value, user_stocks=user_stocks, user_cash=user_cash)
...
def buy():
    ...
    if request.method == "POST":
        ...
        user_cash = db.execute("SELECT cash FROM users WHERE id = 1")
        ...
        if user_cash > (shares * price):
            ...
            update = db.execute("UPDATE users SET cash = :cash WHERE id = 1", cash=user_cash)
...
def history():
    ...
    if request.method == "GET":
        ...
        transactions = db.execute("SELECT * FROM history WHERE id = 1")
...
def sell():
    ...
    if request.method == "POST":
        ...
        user_shares = db.execute("SELECT shares FROM portfolio WHERE id = 1 AND stock = symbol")

        if shares > user_shares:
            return apology("shares exceed currently held", 403)
        ...
        # Process the transaction and store the information in the database
        result = db.execute("DELETE * FROM portfolio WHERE stock = :stock AND id = 1", stock=request.form.get("symbol"))
    ...
    # User reached route via GET (as by clicking a link or via redirect)
        ...
        stocks = db.execute("SELECT stock FROM portfolio WHERE id = 1 GROUP BY stock")
